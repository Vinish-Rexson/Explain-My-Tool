...
DECLARE
    ...
    policy_names text[] := ARRAY[
        'Allow authenticated uploads to demo-assets',
        'Allow users to read own demo assets', 
        'Allow public read access to demo assets',
        'Allow users to update demo assets',
        'Allow users to delete demo assets',
        'Allow users to update own demo assets',
        'Allow users to delete own demo assets'
    ];
BEGIN
...
CREATE POLICY "demo_assets_authenticated_upload"
ON storage.objects
FOR INSERT
TO authenticated
...
CREATE POLICY "demo_assets_authenticated_update"
ON storage.objects
FOR UPDATE
TO authenticated
USING (bucket_id = 'demo-assets');
...
CREATE POLICY "demo_assets_authenticated_delete"
ON storage.objects
FOR DELETE
TO authenticated
USING (bucket_id = 'demo-assets');